<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" class="login-form" :rules="rules" auto-complete="on" label-position="left">
...
<script>
import { vaildPhoto } from '@/utils/validate'
export default {
  name: 'Login',
  data() {
    const photoVaild = (rules, value, callback) => {
      if (!vaildPhoto(value)) {
    ...
    return {
      ...
      rules: {
        mobile: [
          ...
          { validator: photoVaild, trigger: 'blur' }
        ],
        password: [
          { required: true, message: '密码不能为空', trigger: 'blur' },
          { min: 6, max: 16, message: '密码长度必须在6~16位', trigger: 'blur' }
        ]
      }
    }
  },
